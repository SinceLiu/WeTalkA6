		...
		while (cursor.moveToNext()) {
			...
			case StructuredPostal.CONTENT_ITEM_TYPE:
				if(cursor.getInt(cursor.getColumnIndex(StructuredPostal.TYPE)) == StructuredPostal.TYPE_WORK){
					String country = cursor.getString(cursor.getColumnIndex("data8"));
				    contact.uuid = country;
				    int unread = cursor.getInt(cursor.getColumnIndex("data6"));
//				    if(unread != 0){
...
//				    contact.unreadCount = mPrefs.getUserUnreadCount(contact.uuid);
				    ...
				    int relation = cursor.getInt(cursor.getColumnIndex("data9"));
			...
			case Photo.CONTENT_ITEM_TYPE:
				...
				String photoUri = cursor.getString(cursor.getColumnIndex(Photo.PHOTO_URI));
