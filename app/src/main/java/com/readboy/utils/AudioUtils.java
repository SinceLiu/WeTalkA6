...
public class AudioUtils {
    private static final String TAG = "hwj_AudioUtils";
    ...
    /**
     * 申请临时焦点，暂停其他应用播放, 调用{@link #abandonAudioFocus(Context)}可恢复播放。
     * @return 是否抢焦点成功，如果为{@link AudioManager#AUDIOFOCUS_GAIN}则代表抢焦点成功，
     *      其他则不成功。
     */
    public static int requestAudioFocusTransient(Context context) {
        ...
        Log.e(TAG, "requestAudioFocus: get audio service fail");
    ...
    public static int abandonAudioFocus(Context context) {
        ...
        AudioManager sAudioManager =
                (AudioManager) context.getApplicationContext().getSystemService(Context.AUDIO_SERVICE);
        if (sAudioManager != null) {
            int ret = sAudioManager.abandonAudioFocus(null);
            Log.d(TAG, "abandonAudioFocus: ret = " + ret);
            if (ret == AudioManager.AUDIOFOCUS_REQUEST_FAILED) {
        ...
        Log.e(TAG, "abandonAudioFocus: get audio service fail");
        return AudioManager.AUDIOFOCUS_REQUEST_FAILED;
